...
// Date: August 8, 2012
//
// AdminUpdateCalendar.cgi
// Description: AdminUpdateCalendar.cgi is the admin 
// interface for the admin to pick a date to view that week
// of the appointment schedule.
// It shows current month. Admin can choose only on the mondays
// to select the week to view.
// *************************************************************
-->
...
<head>
<title>Dr. Fred's Admin Update Calendar</title>
<!--Calendar script-->
...
<!--Display subtitle-->
  <p style="font-size:16pt;">
    <?php
      echo "Choose specific week to view by clicking on the Monday of the week."
    ?>
  </p>
...
<!--Display appointment calendar-->
  ...
  </div><div id="SelectedDate" style="text-align: center; margin-top: 0.3em">Week of</div>


<form method="post" action="AdminWeekAppointmentSchedule.cgi">
  ...
  <p align=center><input type="submit" value="Select Appointment Week"/><br /></p>
</form>
...
({
    ...
    disabled: function(date) {
        if (date.getDay() == 0 || date.getDay() == 6 || date.getDay() == 2 || date.getDay() == 3 || date.getDay() == 4 || date.getDay() == 5) {
            return true;
        } else {
            return false;
        }
